            ...
            async def wrapper(client, message: Message):
                ...
                if message.from_user:
                    ...
                    if sender_id != current_client_id and not is_sudo_cmd:
                        ...
                        # Use ourselves (User objects) for IDs if available, else fallback to clients (Client.me)
                        other_client_ids = [u.id for u in self.ourselves] if self.ourselves else [c.me.id for c in self.clients if hasattr(c, 'me') and c.me]
    ...
    async def _run(self):
        ...
        while True:
            try:
                ...
                stop_msg = "🛑 Idle stopped, shutting down..."
                self.log(stop_msg)
                
                # ✅ Notif log ke group log if enabled
                try:
                    is_enabled = str(await self.config.get_env("HEARTBEAT_NOTIF_ENABLED", default="off")).lower() == "on"
                    if is_enabled and self.log_chat and hasattr(self, 'bot') and self.bot.is_connected:
                        now_str = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
                        notif_text = (
                            f"<blockquote expandable>"
                            f"<b>🛑 System Shutdown</b>\n"
                            f"━━━━━━━━━━━━━━━━━━\n"
                            f"• <b>Status:</b> {stop_msg}\n"
                    ...
                    self.log(f"Gagal kirim notif shutdown: {e}", level=logging.DEBUG)
    ...
    async def _heartbeat_monitor(self):
        ...
        while True:
            try:
                ...
                # ✅ Notif log ke group log if enabled (v1.0.607 logic)
                try:
                    is_enabled = str(await self.config.get_env("HEARTBEAT_NOTIF_ENABLED", default="off")).lower() == "on"
                    if is_enabled and self.log_chat and hasattr(self, 'bot') and self.bot.is_connected:
                        notif_text = (
                            ...
                            f"• <b>Memori:</b> <code>{mem_mb} MB</code>\n"
                            f"━━━━━━━━━━━━━━━━━━\n"
                            f"<i>Status: Active & Monitoring</i></blockquote>"
                        )
                        await self.bot.send_message(
                            int(self.log_chat),
                            notif_text,
                    ...
                    self.log(f"Gagal kirim notif heartbeat: {e}", level=logging.DEBUG)
        ...
        async def check_bot():
            try:
                if hasattr(self, 'bot') and self.bot.is_connected:
                    ...
                    self.log("✅ Pyrogram Bot: OK")
                    return True
                return False
            except asyncio.TimeoutError:
                self.log("❌ Pyrogram Bot: TIMEOUT", level=logging.ERROR)
                return False
            except Exception as e:
                self.log(f"❌ Pyrogram Bot: FAILED - {e}", level=logging.ERROR)
                return False
        ...
        async def check_user_session(idx, client):
            try:
                if client.is_connected:
                    ...
                    self.log(f"✅ Pyrogram session {idx}: OK")
                    return True
                return False
            except asyncio.TimeoutError:
                self.log(f"❌ Pyrogram session {idx}: TIMEOUT", level=logging.WARNING)
                return False
            except Exception as e:
                self.log(f"❌ Pyrogram session {idx}: FAILED - {e}", level=logging.WARNING)
                return False
            ...
            try:
                ...
                for each_command_data in commands_data:
                    ...
                    if category_text:
                        ...
                        self._command_help_message_data[
                            plugin_name
                        ] += f"   \r<b>➥ Category :</b>  <code>{e_cat}</code>\n"
