...
PLUGIN_VERSION = "1.3.75"  # ✅ Added self-destruct media download+reupload support
...
async def pm_logger_user_handler(c: Client, m: RawMessage):
    ...
    try:
        ...
        # Get file size if restricted or self-destruct
        size_str = ""
        if is_restricted or is_self_destruct:
            size = 0
        ...
        log_content = (
            f"ℹ️ <b>New PM Received (User)</b>"
        )
        if is_self_destruct:
            log_content += f"🔥 <b>Self-Destruct Media (TTL: {ttl_value}s){size_str}</b>\n\n"
        elif is_restricted:
        ...
        if Altruix.log_chat:
            if is_self_destruct:
                ...
                try:
                    logger.info(f"PMLU: Self-destruct media (TTL={ttl_value}s), downloading...")
                    ...
                    if file_path:
                        ...
                        try:
                            if m.photo:
                                reupload_msg = await c.send_photo(
                                ...
                                reupload_msg = await c.send_video(
                                    Altruix.log_chat, file_path, caption=sd_caption,
                                    parse_mode=enums.ParseMode.HTML, message_thread_id=topic_id)
                            elif m.video_note:
                                reupload_msg = await c.send_video_note(
                                    Altruix.log_chat, file_path, message_thread_id=topic_id)
                            elif m.voice:
                                reupload_msg = await c.send_voice(
                                    Altruix.log_chat, file_path, caption=sd_caption,
                                    parse_mode=enums.ParseMode.HTML, message_thread_id=topic_id)
                            else:
                                reupload_msg = await c.send_document(
                                    Altruix.log_chat, file_path, caption=sd_caption,
                                    parse_mode=enums.ParseMode.HTML, message_thread_id=topic_id)
                        except Exception as up_e:
                            logger.error(f"PMLU: Self-destruct re-upload failed: {up_e}")
                        finally:
                            if os.path.exists(str(file_path)):
                                os.remove(file_path)
                        logger.info("PMLU: Self-destruct media saved to log group")
                    else:
                        logger.warning("PMLU: Self-destruct download returned empty path")
                except Exception as e:
                    logger.error(f"PMLU: Self-destruct download failed: {e}")
            else:
                # Normal forward
                try:
                    ...
                    # ✅ Fallback: if forward fails for media, try download+reupload
                    if m.media:
                        try:
                            file_path = await c.download_media(m)
                            if file_path:
                                reupload_msg = await c.send_document(
                                    Altruix.log_chat, file_path,
                                    caption=f"📎 <b>Auto-Saved Media</b>\nFrom: {sender_hyperlink}",
                                    parse_mode=enums.ParseMode.HTML, message_thread_id=topic_id)
                                if os.path.exists(str(file_path)):
                                    os.remove(file_path)
                        except Exception as dl_e:
                            logger.error(f"PMLU Fallback reupload failed: {dl_e}")

        if not fwd_msg and not reupload_msg and not Altruix.log_chat:
            return

        # Send Detailed Info using Main Bot (Altruix.bot) for robustness
...
async def pm_logger_user_edit_handler(c: Client, m: RawMessage):
    ...
    try:
        ...
        log_content = (
            f"ℹ️ <b>New PM Received (User) [#EDITED]</b>\n"
            f"<blockquote expandable>• <b>From:</b> {sender_hyperlink}\n"
            f"• <b>User ID:</b> <code>{sender_id}</code>\n"
            f"• <b>Username:</b> {sender_username}\n"
            f"• <b>To Account:</b> {c.me.mention}\n"
