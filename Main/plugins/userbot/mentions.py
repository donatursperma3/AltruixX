    ...
    try:
        ...
        for wid, data in REPLY_AS_MENTIONED_WAITING.items():
            if data.get("instruction_msg_id") == reply_msg_id:
                waiting_id = wid
                logger.info(f"✅ [DEBUG] Match found by direct ID: {waiting_id}")
                break
        
        # 🔥 **PERBAIKAN**: Jika tidak ditemukan di memory, coba cari di persistent cache
        if not waiting_id:
            logger.info(f"🔍 [DEBUG] Direct match failed for {reply_msg_id}. Checking all entries...")
            # Coba cari berdasarkan instruction_msg_id di seluruh cache memory
            for wid, data in REPLY_AS_MENTIONED_WAITING.items():
                if data.get("instruction_msg_id") == reply_msg_id:
                    waiting_id = wid
                    logger.info(f"✅ [DEBUG] Match found after re-scanning memory: {waiting_id}")
                    break
