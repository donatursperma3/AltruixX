        ...
        try:
            ...
            for dest in destinations:
                try:
                    ...
                    logger.info(f"[CreateGroup] ✅ Notifikasi recovery dikirim ke {dest} untuk Task {key_id}")
                except Exception as send_err:
                    logger.warning(f"[CreateGroup] Gagal kirim recovery ke {dest} untuk Task {key_id}: {send_err}")
                    
            RECOVERY_NOTIFIED.add(key_id)
        except Exception as e:
            logger.error(f"[CreateGroup] ❌ Gagal memproses notifikasi recovery untuk {key_id}: {e}")
...
def get_creategroup_dashboard_text(tid: str):
    ...
    text = (
        ...
        f"• <b>Status:</b> {status}\n"
        f"• <b>Log Destination:</b> <code>{log_dest_lbl}</code>\n"
...
async def get_creategroup_keyboard(client_id: int, tid: str, menu_type: str = "main"):
    ...
    if menu_type == "manage":
        ...
        pm_bot_lbl = "PM Bot" + (" ✅" if log_dest == "pm_bot" else "")
        log_group_lbl = "Group Log" + (" ✅" if log_dest == "log_group" else "")
        both_lbl = "Both" + (" ✅" if log_dest == "both" else "")
        disable_lbl = "Disable" + (" ✅" if log_dest == "off" else "")
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton(await Essentials.get_user_button_style(client_id, "🔄 Refresh Logs"), 
    ...
    try:
        ...
        if is_resume:
            ...
            await send_start_log(
                ...
                f"• Task ID: <code>{tid}</code>\n"
                f"• Account: <b>{html.escape(f'{user_info.first_name or str()} {user_info.last_name or str()}'.strip() or 'Unknown')}</b>\n"
    ...
    try:
            ...
            for bot_id in bot_identifiers:
                try:
                    ...
                    await send_log_notification(
                        bot_client,
                        f"<blockquote expandable>"
                        f"⚠️ Gagal mendapatkan bot {bot_id} (Akun: {account_name_raw}): {str(e)}"
                        f"</blockquote>",
                        effective_user_id,
                        reply_id
                    )
        ...
        while i <= count:
            ...
            if state.get("paused", False):
                ...
                await send_log_notification(
                    bot_client,
                    f"<blockquote expandable>▶️ Task Create {type_label} di-resume pada {unit_label} ke-{i}\n• Account: {html.escape(account_name)}</blockquote>",
                    effective_user_id,
                    reply_id
                )
                ...
                if current_step == "create":
                    ...
                    if group_type == "b":
                        ...
                        try:
                            ...
                            error_msg = (
                                f"<blockquote expandable>"
                                f"❌ <b>Gagal membuat Grup Dasar</b>\n"
                                f"• Account: {html.escape(account_name_raw)}\n"
