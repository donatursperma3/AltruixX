...
@Altruix.register_on_cmd(
    ["syncenv", "senv"],
    cmd_help={
        "help": "Sync .env file variables to Database config.",
        "usage": ".syncenv <optional: KEY / 'all'>",
        "example": ".syncenv BOT_TOKEN",
        "detail": (
            "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "⚙️ **ENV SYNCHRONIZER**\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "Since Database variables have higher priority than .env, "
            "use this to update your DB from your .env file.\n\n"
            "• `.syncenv` : View keys that differ.\n"
            "• `.syncenv all` : Sync EVERYTHING from .env to DB.\n"
            "• `.syncenv KEY` : Sync a specific key (e.g. BOT_TOKEN)."
        )
    },
)
@iuser_check
@log_errors
async def sync_env_handler(c: Client, m: Message):
    """Utility to sync .env to DB."""
    import dotenv
    env_file = ".env"
    if not os.path.exists(env_file):
        return await m.reply_msg("❌ <b>.env file not found!</b>")

    env_vars = dotenv.dotenv_values(env_file)
    target = m.user_input.strip() if m.user_input else ""
    
    if target.lower() == "all":
        count = 0
        for k, v in env_vars.items():
            if v:
                await Altruix.config.sync_env_to_db(k, v, upsert=True)
                count += 1
        return await m.reply_msg(f"✅ <b>Synced {count} variables to Database!</b>\n<i>Restart may be required for some changes.</i>")
    ...
    if target:
        ...
        if val is None:
            return await m.reply_msg(f"❌ <b>Key '{key}' not found in .env!</b>")
        
        await Altruix.config.sync_env_to_db(key, val, upsert=True)
        return await m.reply_msg(f"✅ <b>SYNCED:</b> <code>{key}</code> updated in Database.")

    # No args: List differences
    diff_msg = "<b>🔍 .env vs Database Sync Check</b>\n\n"
    diff_count = 0
    for k, v in env_vars.items():
        db_v = await Altruix.config.get_env(k)
        if str(v) != str(db_v):
            diff_msg += f"• <code>{k}</code>: <i>(Click to sync)</i>\n"
            diff_count += 1
            if diff_count >= 15: break # Limit output
            
    if diff_count == 0:
        return await m.reply_msg("✅ <b>Database is already in sync with .env!</b>")
    
    diff_msg += f"\nFound <b>{diff_count}</b> differences.\nUse <code>.syncenv all</code> to sync everything."
    await m.reply_msg(diff_msg)
