...
async def purgeme_cmd(client: Client, message: Message):
    ...
    # ✅ CROSS-EXECUTION PREVENTION (Refined Leak Fix)
    # 1. If command is from an alliance session, only the sender should execute it.
    # 2. If command is from a sudo user, only Client #0 executes it by default to avoid mass-triggering.
    if not message.outgoing:
        # Robust ID collection from all connected sessions
        alliance_ids = [u.id for u in Altruix.ourselves] if hasattr(Altruix, "ourselves") else []
        if not alliance_ids: # Fallback to clients if ourselves is empty
            alliance_ids = [c.me.id for c in Altruix.clients if hasattr(c, 'me') and c.me]
        
        sender_id = message.from_user.id
        current_id = client.me.id
        
        # 🟢 DEBUG LOGGING: Identify why leaks happen
        Altruix.log(f"PURGEME_DEBUG: [Sender: {sender_id}] [Current: {current_id}] [Alliance: {alliance_ids}]", level=10)

        if sender_id in alliance_ids:
            if sender_id != current_id:
                # This is another alliance session's command; skip to prevent leak.
                return 
        else:
            # External Sudo User: Only let Client #0 respond to prevent mass-configuring
            if Altruix.clients and client != Altruix.clients[0]:
                return
    ...
    try:
        ...
        if Altruix.log_chat and state.get("status") in ["collecting", "running", "paused", "waiting", "finished", "stopped"]:
            ...
            try:
                ...
                # ✅ FIX: For log chat updates, ALWAYS use main bot assistant (it's guaranteed to be a member)
                # Session-specific custom bots may not have invitation to the log group.
                custom_bot = Altruix.bot 
...
async def purgeme_pause_cmd(client: Client, message: Message):
    ...
    # If not a self-message, only react if we actually have an active task for this chat.
    # Otherwise, fallback to Client #0 for "No active purge" notifications.
    uid_check = f"{message.chat.id}_{client.me.id}"
    has_active_task = uid_check in Altruix.PURGEME_STATE
    if not is_self:
        if not has_active_task and Altruix.clients and client != Altruix.clients[0]:
            return
