...
# Menggunakan handler dari config client alih-alih mencoba akses atribut yang tidak ada
try:
...
# Coba berbagai cara untuk mendapatkan LOG_CHAT_ID
try:
                    ...
                    if old_purge_val > 0:
                        try:
                            ...
                            if messages:
                                ...
                                await USER_CLIENT.send_message(
                                    LOG_CHAT_ID,
                                    f"[HAPUS]: Berhasil hapus {del_suk} pesan lama, gagal {del_ggl} pesan.",
...
async def start_relayspam(client: Client, destination: str, start_delay: float, stop_delay: float, 
    ...
    try:
        ...
        notif_msg = (
            ...
            f"Hapus      : {old_purge} pesan\n"
            f"React      : {emot_react if emot_react else 'tidak ada'}\n"
            f"Pesan      : {msg_list}\n"
        )
...
async def telayspammer_cmd(c: Client, m: Message):
    ...
    try:
        ...
        # Pattern untuk menghapus command dan prefix
        ...
        # Format: <destination> <start_delay> <stop_delay> <step> <count> <purge> <emot_react/optional>
        # Jadi minimal 6 argumen, maksimal 7
