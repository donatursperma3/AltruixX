        ...
        try:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                ...
                if "start_id" not in columns:
                    await db.execute("ALTER TABLE fwd_tasks ADD COLUMN start_id INTEGER DEFAULT 1")
                if "end_id" not in columns:
                    await db.execute("ALTER TABLE fwd_tasks ADD COLUMN end_id INTEGER DEFAULT 1")
                if "batch_msg" not in columns:
                ...
                if "default_batch_msg" not in g_columns:
                    await db.execute("ALTER TABLE fwd_global_settings ADD COLUMN default_batch_msg INTEGER DEFAULT 30")
                if "default_batch_msg_delay" not in g_columns:
                    await db.execute("ALTER TABLE fwd_global_settings ADD COLUMN default_batch_msg_delay REAL DEFAULT 0.0")
...
async def safe_edit_cb_text(cb, text: str, reply_markup=None, parse_mode=None):
    """Safely edit a callback message or inline message. Returns True on success."""
    try:
        is_inline = bool(getattr(cb, 'inline_message_id', None))
        if is_inline:
            await Altruix.bot.edit_inline_text(inline_message_id=cb.inline_message_id, text=text, reply_markup=reply_markup, parse_mode=parse_mode)
        else:
            await cb.edit_message_text(text, reply_markup=reply_markup, parse_mode=parse_mode)
        return True
    except Exception as e:
        logger.debug(f"[ForwardPro] safe_edit_cb_text failed: {e}")
        return False
...
async def safe_cb_ack(cb: CallbackQuery):
    """Acknowledge callback quietly."""
    try:
        await cb.answer()
    except Exception:
        pass
...
def _normalize_delay(value, default: float = 1.5) -> float:
    """Normalize various delay inputs to float with fallback."""
    try:
        if value is None:
...
def build_task_detail_kb(task: dict, user_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("📦 CONFIG BATCH RANGE", callback_data=f"fwd_btchrange_{tid}_{user_id}", style=btn_style)
        ],
        [
...
def build_advanced_options_kb(task: dict, user_id: int) -> InlineKeyboardMarkup:
    ...
    batch_msg = int(task.get("batch_msg", 30) or 30)
    batch_msg_delay = _normalize_delay(task.get("batch_msg_delay", 0.0), 0.0)
    ...
    return InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton(f"B.Msg Delay: {batch_msg_delay}s", callback_data=f"fwd_abmsgdelay_menu_{tid}_{user_id}", style=btn_style)
        ],
        [
...
async def build_advanced_options_text(task: dict) -> str:
    ...
    text = (
        ...
        f"  • Batch Msg: <b>{task.get('batch_msg', 30)}</b>\n"
...
async def fwd_callback_handler(client: Client, cb: CallbackQuery):
    ...
    # ── BATCH MSG / BATCH MSG DELAY (Global & Task) ──
    if action in ("gbmsg", "gbmsgdelay", "abmsg", "abmsgdelay"):
        is_inline = bool(cb.inline_message_id)
        inl_id = cb.inline_message_id if is_inline else None

        sub = parts[2] if len(parts) > 2 else "menu"
        is_task = action.startswith("a")
        ...
        if is_task:
            ...
            if not task:
                return await safe_cb_answer(cb, "❌ Task not found.", show_alert=True)
            cur_msg = int(task.get("batch_msg", 30) or 30)
            cur_delay = _normalize_delay(task.get("batch_msg_delay", 0.0), 0.0)
            scope = f"Task #{tid}"
            back_cbd = f"fwd_advopts_{tid}_{uid}"
        else:
            gs = await get_global_settings(uid)
            cur_msg = int(gs.get("default_batch_msg", 30) or 30)
            cur_delay = _normalize_delay(gs.get("default_batch_msg_delay", 0.0), 0.0)
            scope = "Global"
            back_cbd = f"fwd_gadvopts_{uid}"

        # Message count menu
        if action in ("gbmsg", "abmsg"):
            steps = [1, 5, 10, 30, 50, 100]
            if sub in ("inc", "dec"):
                try:
                    step = int(parts[3])
                except Exception:
                    step = 0
                if step > 0:
                    cur_msg = min(1000, cur_msg + step) if sub == "inc" else max(1, cur_msg - step)
                    if is_task:
                        await update_task(tid, batch_msg=cur_msg)
                    else:
                        await update_global_settings(uid, default_batch_msg=cur_msg)
                    await safe_cb_answer(cb, f"✅ {scope} Batch Msg: {cur_msg}", show_alert=False)

            header = f"🧮 <b>{scope} Batch Msg</b>\n{'━' * 18}\nAtur jumlah pesan per batch (cooldown) untuk mode batch.\n\nCurrent: <b>{cur_msg}</b>\n{'━' * 18}"
            text = f"<blockquote expandable>{header}</blockquote>"

            btn_style = get_user_button_style(uid)
            rows = [[InlineKeyboardButton(f"━━ Batch Msg: {cur_msg} ━━", callback_data="noop", style=btn_style)]]
            for s in steps:
                rows.append([
                    InlineKeyboardButton(f"-{s}", callback_data=(f"fwd_{action}_dec_{s}_{tid}_{uid}" if is_task else f"fwd_{action}_dec_{s}_{uid}"), style=btn_style),
                    InlineKeyboardButton(f"+{s}", callback_data=(f"fwd_{action}_inc_{s}_{tid}_{uid}" if is_task else f"fwd_{action}_inc_{s}_{uid}"), style=btn_style),
                ])
            rows.append([InlineKeyboardButton("🔙 Back", callback_data=back_cbd, style=btn_style)])
            ...
            if sub in ("inc", "dec"):
                try:
                    ...
                    step = 0.0
                if step > 0:
                    cur_delay = min(3600.0, cur_delay + step) if sub == "inc" else max(0.0, cur_delay - step)
                    if is_task:
                        await update_task(tid, batch_msg_delay=cur_delay)
                    else:
                        await update_global_settings(uid, default_batch_msg_delay=cur_delay)
                    await safe_cb_answer(cb, f"✅ {scope} Batch Msg Delay: {cur_delay}s", show_alert=False)

            header = f"⏱ <b>{scope} Batch Msg Delay</b>\n{'━' * 18}\nAtur jeda antar batch (cooldown) untuk mode batch.\n\nCurrent: <b>{cur_delay}s</b>\n{'━' * 18}"
            text = f"<blockquote expandable>{header}</blockquote>"

            btn_style = get_user_button_style(uid)
            rows = [[InlineKeyboardButton(f"━━ Batch Msg Delay: {cur_delay}s ━━", callback_data="noop", style=btn_style)]]
            for s in steps:
                rows.append([
                    InlineKeyboardButton(f"-{int(s)}s", callback_data=(f"fwd_{action}_dec_{int(s)}_{tid}_{uid}" if is_task else f"fwd_{action}_dec_{int(s)}_{uid}"), style=btn_style),
                    InlineKeyboardButton(f"+{int(s)}s", callback_data=(f"fwd_{action}_inc_{int(s)}_{tid}_{uid}" if is_task else f"fwd_{action}_inc_{int(s)}_{uid}"), style=btn_style),
                ])
            rows.append([InlineKeyboardButton("🔙 Back", callback_data=back_cbd, style=btn_style)])
            kb = InlineKeyboardMarkup(rows)

        await safe_edit_cb_text(cb, text, reply_markup=kb, parse_mode=enums.ParseMode.HTML)
        return
    ...
    # ── BATCH RANGE MENU ──
    if action in ("btchrange", "br"):
        is_inline = bool(cb.inline_message_id)
        inl_id = cb.inline_message_id if is_inline else None
        is_adjustment = (action == "br")

        if is_adjustment:
            # fwd_br_{start|end|run}_{inc|dec|tid}_{val|uid}_{tid}_{uid}
            ...
            if sub == "run":
                try:
                    ...
                    return await safe_cb_answer(cb, "❌ Invalid callback format.", show_alert=True)
                op = "run"
            else:
        ...
        if not is_adjustment:
            await safe_cb_answer(cb, "📦 Membuka Batch Range…", show_alert=False)

        # ── EXECUTE BATCH RUN ──
        if sub == "run":
            start_id = int(task.get("start_id", 1))
            end_id = int(task.get("end_id", 1))

            if start_id > end_id:
                return await safe_cb_answer(cb, "❌ Start ID cannot be greater than End ID.", show_alert=True)

            await safe_cb_answer(cb, f"🚀 Executing Batch Task #{tid} ({start_id} - {end_id})...", show_alert=True)

            # Use predictable ID: #FPB{tid}
            reg_tid = f"#FPB{tid}"
            asyncio.create_task(start_batch_fwd(ub_client, uid, tid, start_id, end_id, reg_tid=reg_tid))

            # Refresh detail view
            text = await build_task_detail_text(task, client=ub_client)
            kb = build_task_detail_kb(task, uid)
            ok = await safe_edit_cb_text(cb, text, reply_markup=kb, parse_mode=enums.ParseMode.HTML)
            if not ok:
                logger.error(f"Batch Range Run UI Error: failed to edit UI for Task #{tid}")
            return

        cur_start = int(task.get("start_id", 1))
        cur_end = int(task.get("end_id", 1))

        if is_adjustment and op in ("inc", "dec"):
            try:
                val = int(parts[4])
                if sub == "start":
                    cur_start = max(1, cur_start + val if op == "inc" else cur_start - val)
                    await update_task(tid, start_id=cur_start)
                else:
                    cur_end = max(1, cur_end + val if op == "inc" else cur_end - val)
                    await update_task(tid, end_id=cur_end)
                await safe_cb_answer(cb, f"✅ Updated {sub.title()}: {cur_start if sub == 'start' else cur_end}")
            except Exception as e:
                logger.error(f"Batch Range Adjust Error: {e}")
        ...
        # Start ID row
        rows.append([InlineKeyboardButton(f"━━ START ID: {cur_start} ━━", callback_data="noop", style=btn_style)])
        for i in range(0, len(steps), 2):
            s1, s2 = steps[i], steps[i+1]
            rows.append([
                InlineKeyboardButton(f"-{s2}", callback_data=f"fwd_br_start_dec_{s2}_{tid}_{uid}", style=btn_style),
                InlineKeyboardButton(f"-{s1}", callback_data=f"fwd_br_start_dec_{s1}_{tid}_{uid}", style=btn_style),
                InlineKeyboardButton(f"+{s1}", callback_data=f"fwd_br_start_inc_{s1}_{tid}_{uid}", style=btn_style),
                InlineKeyboardButton(f"+{s2}", callback_data=f"fwd_br_start_inc_{s2}_{tid}_{uid}", style=btn_style),
            ])

        # End ID row
        rows.append([InlineKeyboardButton(f"━━ END ID: {cur_end} ━━", callback_data="noop", style=btn_style)])
        for i in range(0, len(steps), 2):
            s1, s2 = steps[i], steps[i+1]
            rows.append([
                InlineKeyboardButton(f"-{s2}", callback_data=f"fwd_br_end_dec_{s2}_{tid}_{uid}", style=btn_style),
                InlineKeyboardButton(f"-{s1}", callback_data=f"fwd_br_end_dec_{s1}_{tid}_{uid}", style=btn_style),
                InlineKeyboardButton(f"+{s1}", callback_data=f"fwd_br_end_inc_{s1}_{tid}_{uid}", style=btn_style),
                InlineKeyboardButton(f"+{s2}", callback_data=f"fwd_br_end_inc_{s2}_{tid}_{uid}", style=btn_style),
            ])

        rows.append([InlineKeyboardButton("🚀 EXECUTE BATCH", callback_data=f"fwd_br_run_{tid}_{uid}", style=btn_style)])
        rows.append([InlineKeyboardButton("🔙 Back", callback_data=f"fwd_detail_{tid}_{uid}", style=btn_style)])

        kb = InlineKeyboardMarkup(rows)
        ok = await safe_edit_cb_text(cb, header, reply_markup=kb, parse_mode=enums.ParseMode.HTML)
        if not ok:
            logger.error("Batch Range Menu UI Error: failed to edit UI")
            await safe_cb_answer(cb, "❌ Gagal membuka menu Batch Range.", show_alert=True)
        return
