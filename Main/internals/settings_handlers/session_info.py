...
async def get_session_info_data(index: int, page: int = 1, button_page: int = 1, refresh: bool = False):
    ...
    status_icon = "ACTIVE" if not is_disabled else "DISABLED"
    ...
    # All available features (Total: 53)
    # Format: (key, data, label_override)
    all_features = [
        # Page 1 features
        ...
        ("download_story", f"gen_conf_dlstory_session_input_{index}_{page}", None),
        ("download_content", f"gen_conf_dl_content_input_{index}_{page}", None), 
        ("download_user_photo", f"gen_conf_dl_uphoto_start_{index}_{page}", None),
        ("purge_my_msg", f"purge_msg_start_{index}_{page}", None), 
        ("send_message", f"gen_conf_send_message_input_{index}_{page}", None),
        ("join_group", f"gen_conf_join_chat_input_{index}_{page}", None), 
        ("leave_group", f"gen_conf_leave_chat_input_{index}_{page}", None),
        ("track_profile", f"gen_conf_track_profile_{index}_{page}", None),
        # Page 3 features
        ("chat_stats", f"gen_conf_chat_stats_scan_{index}_{page}", None),
        ("creategroup_menu", f"creategroup_menu_{index}_{page}", None), 
        ("recent_messages", f"recent_msgs_menu_{index}_{page}", None),
        ("view_mentions", f"view_mnt_menu_{index}_{page}", None), 
        ("pm_logger_control", f"pml_menu_{index}_{page}", None),
        ("mention_control", f"mnt_menu_{index}_{page}", None), 
        ("join_logger_control", f"joinl_menu_{index}_{page}", None),
        ("cmd_logger_control", f"cmdl_menu_{index}_{page}", None), 
        ("privacy_security", f"privacy_menu_{index}_{page}", None),
        ("startup_settings_title", f"startup_menu_{index}_{page}", None), 
        # Page 4 features
        ("cmd_settings", f"cmd_settings_menu_{index}_{page}", None),
        ("sudo_settings", f"sudo_menu_{index}_{page}", None), 
        ("prefix_settings", f"prefix_menu_{index}_{page}", None),
        ("prefix_info", f"prefix_info_{index}_{page}", None), 
        ("feature_status", f"feature_status_{index}_{page}", None),
        ("export_session", f"gen_conf_export_session_{index}_{page}", None), 
        ("export_phone", f"gen_conf_export_phone_{index}_{page}", None),
        ("eval_python", f"gen_conf_eval_exec_{index}_{page}", None), 
        ("exec_terminal", f"gen_conf_exec_term_{index}_{page}", None),
        ("env_manager", f"env_manager_list_1", None), 
        # Page 5 features
        ("change_login_email", f"change_login_email_{index}_{page}", None),
        ("custom_bot", f"custom_bot_menu_{index}_{page}_1", None), 
        ("auto_gp_menu", f"auto_gp_menu_{index}_{page}", "Auto GP"),
        ("btn_gcast_user", f"gcast_user_{index}_{page}", None),
        ("global_purgeme", f"global_purgeme_{index}_{page}", None),
        ("sys_ctrl_restart", f"sys_ctrl_restart", None),
        ("sys_ctrl_shutdown", f"sys_ctrl_shutdown", None),
        ("help_settings_menu", f"help_settings_menu_{index}_{page}", "Custom Help"),
        ("custom_alert_menu", f"custom_alert_menu_{index}_{page}", "Custom Alert"),
    ...
    # 1. Toggle Session (ALWAYS at the top)
    buttons.append([InlineKeyboardButton(f"{Altruix.get_string('toggle_session_status') or 'Toggle Session'}", f"toggle_session_confirm_{index}_{page}", style=user_style)])
    
    # 2. Page features (2 per row, max 5 rows = 10 buttons)
    items_per_page = 10
    total_pages = (len(all_features) + items_per_page - 1) // items_per_page
    if button_page > total_pages: button_page = total_pages
    
    start_idx = (button_page - 1) * items_per_page
    end_idx = start_idx + items_per_page
    page_items = all_features[start_idx:end_idx]
    
    for i in range(0, len(page_items), 2):
        row = []
        # First button in row
        key1, data1, override1 = page_items[i]
        label1 = override1 or Altruix.get_string(key1) or key1.replace("_", " ").title()
        row.append(InlineKeyboardButton(label1, data1, style=user_style))
        
        # Second button in row
        if i + 1 < len(page_items):
            key2, data2, override2 = page_items[i+1]
            label2 = override2 or Altruix.get_string(key2) or key2.replace("_", " ").title()
            row.append(InlineKeyboardButton(label2, data2, style=user_style))
        buttons.append(row)
    ...
    nav_row.append(InlineKeyboardButton(f"{button_page}/{total_pages}", f"session_info_{index}_{page}_{button_page}", style=user_style))
